...
export class TradingController {
   ...
   */
  static async getTradingBots(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
   ...
   */
  static async createTradingBot(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      res.status(500).json({ success: false, message: 'Failed to create trading bot' });
    }
  }
   ...
   * Update trading bot
   */
  static async updateTradingBot(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      ...
      if (bot.count === 0) {
        res.status(404).json({ success: false, message: 'Trading bot not found' });
        return;
      }
      ...
      res.status(500).json({ success: false, message: 'Failed to update trading bot' });
    }
  }
   ...
   * Delete trading bot
   */
  static async deleteTradingBot(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      ...
      res.status(500).json({ success: false, message: 'Failed to delete trading bot' });
    }
  }
   ...
   */
  static async getCopyTrades(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      // Get user ID from address
      const user = await prisma.user.findUnique({
        where: { address: req.user!.address }
      ...
      res.status(500).json({ success: false, message: 'Failed to get copy trades' });
    }
  }
   ...
   * Set up copy trading
   */
  static async setupCopyTrading(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { targetAddress, allocation, maxSlippage } = req.body;

      if (!targetAddress || !allocation) {
        res.status(400).json({ success: false, message: 'Target address and allocation are required' });
        return;
      }
      ...
      res.status(500).json({ success: false, message: 'Failed to set up copy trading' });
    }
  }
   ...
   */
  static async updateCopyTrading(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      ...
      if (copyTrade.count === 0) {
        res.status(404).json({ success: false, message: 'Copy trading setting not found' });
        return;
      }
      ...
      res.status(500).json({ success: false, message: 'Failed to update copy trading' });
    }
  }
   ...
   */
  static async deleteCopyTrading(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      ...
      res.status(500).json({ success: false, message: 'Failed to delete copy trading' });
    }
  }
} 
        ...
        <div className="flex flex-1 flex-col bg-muted rounded-sm">
          ...
          <div className="flex items-center border-t">
            {[0.01, 0.05, 0.1, 0.2].map((p, index) => (
              <div className={`flex items-center gap-2 cursor-pointer flex-1 justify-center border-r border-white/10 last:border-r-0 py-1 text-sm text-white/50 hover:text-white`} key={index} onClick={() => setAmount(p)} >
                {p}
              </div>
            ))}
          </div>
